<template>
	<view class='index'>
		...
		<view class='navbar' :style="'height:'+navH+'rpx'">
			<view class='navbarH' :style="'height:'+navH+'rpx'">
				<view class='navbarCon acea-row row-center-wrapper'>
					<view class='search acea-row row-middle'><text class='iconfont icon-xiazai5'></text>搜索商品</view>
					<view class='logo'>
		...
		<!-- #endif -->
		<view class='index-bg' v-if="imgUrls.length">
		...
		<view class='news acea-row row-between-wrapper' v-if="itemNew.length">
			<view class='pictrue'>
				 ...
				 circular="true">
					<block v-for="(item,index) in itemNew" :key="index">
						<swiper-item catchtouchmove='catchTouchMove'>
							<navigator v-if="item.show == '1'" :url='item.url' open-type='switchTab' class='acea-row row-between-wrapper'
							 hover-class='none'>
								<view class='text acea-row row-between-wrapper'>
									<view class='label border2' v-if="item.id%2 == 0">最新</view>
									<view class='newsTitle line1'>{{item.info}}</view>
								</view>
								<view class='iconfont icon-xiangyou'></view>
							</navigator>
							<navigator v-else open-type='navigate' :url='item.url' class='acea-row row-between-wrapper' hover-class='none'>
								<view class='text acea-row row-between-wrapper'>
									<view class='label border2' v-if="item.id%2 == 0">最新</view>
									<view class='newsTitle line1'>{{item.info}}</view>
								</view>
		...
		<view class='specialArea acea-row row-between-wrapper' v-if="activityList.length">
			<block v-for="(item,index) in activityList" :key="index">
				<navigator class='assemble' :url='item.link' v-if="index < 1">
					<image :src='item.pic'></image>
					<view class='text'>
			...
			<view class='list acea-row row-column-between'>
				<block v-for="(item,index) in activityList" :key="index">
					<navigator class='item' :url='item.link' v-if="index > 0">
						<image :src='item.pic'></image>
						<view class='text'>
							<view class='name'>{{item.title}}</view>
							<view class='infor'>{{item.info}}</view>
		...
		<view class='wrapper' v-if="fastList.length">
			...
			<view class='scroll-product'>
				<scroll-view class="scroll-view_x" scroll-x style="width:auto;overflow:hidden;">
					<block v-for="(item,index) in fastList" :key="index">
						<navigator hover-class="none" class='item' :url='"/pages/goods_list/index?sid="+item.id+"&title="+item.cate_name'>
							<view class='img-box'>
								<image :src='item.pic'></image>
							</view>
		...
		<view class='wrapper' v-if="bastList.length">
			<view class='title acea-row row-between-wrapper'>
				<view class='text'>
					<view class='name line1'>精品推荐</view>
					<view class='line1'>{{bastInfo}}</view>
				</view>
				<navigator class='more' hover-class="none" url='/pages/first_new_product/index?type=1'>更多<text class='iconfont icon-jiantou'></text></navigator>
			</view>
			<view class='boutique'>
		...
		<view class='hotList' v-if="likeInfo.length">
			<view class='hot-bg'>
				<view class='title acea-row row-between-wrapper'>
					<view class='text line1'><text class='label'>热门榜单</text>根据销量、搜索、好评等综合得出</view>
					<navigator class='more' hover-class="none" url='/pages/first_new_product/index?type=2'>更多<text class="iconfont icon-jiantou"></text></navigator>
				</view>
			</view>
			<view class='list acea-row row-middle'>
				<block v-for="(item,index) in likeInfo" :key="index">
		...
		<view class='adver' v-if="newGoodsBananr">
			<block>
		...
		<view class='wrapper' v-if="firstList.length">
			<view class='title acea-row row-between-wrapper'>
				...
				<navigator class='more' hover-class="none" url='/pages/first_new_product/index?type=3'>更多<text class='iconfont icon-jiantou'></text></navigator>
			</view>
			<view class='newProducts'>
				<scroll-view class="scroll-view_x" scroll-x style="width:auto;overflow:hidden;">
					<block v-for="(item,index) in firstList" :key="index">
						<view hover-class="none" @tap="goDetail(item)" class='item'>
							<view class='img-box'>
								<image :src='item.image'></image>
								<span
		...
		<view class='wrapper' v-if="benefit.length">
			<view class='title acea-row row-between-wrapper'>
				...
				<navigator class='more' hover-class="none" url='/pages/promotional_items/index'>更多<text class='iconfont icon-jiantou'></text></navigator>
			</view>
			<promotionGood :benefit="benefit"></promotionGood>
		</view>
		<couponWindow :window='window' :couponList='couponList' @onColse='onColse'></couponWindow>
	</view>
</template>
<script>
	let app = getApp();
	...
	export default {
		...
		methods: {
			// #ifdef MP-WEIXIN
			getTemlIds() {
				let messageTmplIds = this.$Cache.get(SUBSCRIBE_MESSAGE);
				if (!messageTmplIds) {
					getTemlIds().then(res => {
						if (res.data)	
							this.$Cache.set(SUBSCRIBE_MESSAGE, JSON.stringify(res.data));
					})
				}
			},
			closeTip: function() {
			...
			getIndexConfig: function() {
				...
				getIndexData().then(res => {
					...
					that.$set(that, "itemNew", res.data.roll);
					that.$set(that, "activityList", res.data.activity);
					that.$set(that, "bastBanner", res.data.info.bastBanner);
					that.$set(that, "bastInfo", res.data.info.bastInfo);
					that.$set(that, "bastList", res.data.info.bastList);
					that.$set(that, "fastInfo", res.data.info.fastInfo);
					that.$set(that, "fastList", res.data.info.fastList);
					that.$set(that, "firstInfo", res.data.info.firstInfo);
					that.$set(that, "firstList", res.data.info.firstList);
					that.$set(that, "salesInfo", res.data.info.salesInfo);
					that.$set(that, "likeInfo", res.data.likeInfo);
					that.$set(that, "benefit", res.data.benefit);
					that.$set(that, "logoUrl", res.data.logoUrl);
					that.$set(that, "couponList", res.data.couponList);
					that.$set(that, "newGoodsBananr", res.data.newGoodsBananr);
					// #ifdef H5
